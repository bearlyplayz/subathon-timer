...
function log(...args){ if (F.consoleLog) console.log("[Subathon]", ...args); }
function debug(...args){ if (F.debugLog) console.debug("[Subathon]", ...args); }

/* -------------------- Configuration -------------------- */
let F = {}; // fields
let channel="";
let stateKey=null; // optional storage key (unused in sandbox)
...
function showEvent(message, effect) {
  const box = $('#event');
  if (!box) return;
  if (!message && !effect) { box.classList.add('hide'); return; }

  let msgEl = box.querySelector('.event-message');
  let effEl = box.querySelector('.event-effect');
  if (!msgEl) { msgEl = document.createElement('span'); msgEl.className = 'event-message'; box.appendChild(msgEl); }
  if (!effEl) { effEl = document.createElement('span'); effEl.className = 'event-effect'; box.appendChild(effEl); }
  ...
  eventHideTO = setTimeout(() => box.classList.add('hide'), Number(F.eventFadeMs || 4500));
}

/* -------------------- Event Messages -------------------- */
function eventMessageFor(kind) {
  switch (kind) {
    case 'follow': return F.followMessageText || 'New follower!';
    case 'cheer': return F.cheerMessageText || 'Cheer!';
    case 'sub:T1': return F.subTier1MessageText || 'Tier 1 Sub!';
    case 'sub:T2': return F.subTier2MessageText || 'Tier 2 Sub!';
    case 'sub:T3': return F.subTier3MessageText || 'Tier 3 Sub!';
    case 'sub:Other': return F.subOtherMessageText || 'Subscription!';
    default: return typeof kind === 'string' ? kind : '';
  }
}

/* -------------------- Timer core -------------------- */
function applyDelta(deltaSec, { kind="manual", message="", record=true } = {}) {
  ...
  if (record && deltaSec) {
    eventHistory.unshift({ ts: Date.now(), delta: Number(deltaSec), kind, message });
...
function addSeconds(sec, effectLabel, kind=null) {
  ...
  if (!add) return;
  applyDelta(add, { kind: kind || "manual", message: effectLabel, record: true });
...
function revert(n=1) {
  ...
  while (n-- > 0 && eventHistory.length) {
    ...
    applyDelta(-ev.delta, { kind: "revert", message: `Revert ${ev.kind}: ${ev.delta}s`, record:false });
  }
  showEvent("Reverted", "");
}
...
window.addEventListener("onWidgetLoad", async (obj) => {
  ...
  const { channelData, fieldData } = obj?.detail ?? {};
  channel = channelData?.username || channelData?.providerId || "";
  F = fieldData || {};
  stateKey = channel ? `subathon:${channel}` : null;
...
window.addEventListener("onEventReceived", (obj) => {
  ...
  if (listener === "message" && ev?.data?.text) {
    ...
    const isBroadcaster = (ev.data?.badges || []).some(b => b.type === "broadcaster");
    ...
    if (canControl && text.startsWith("!")) {
      ...
      if ((F.broadcasterSet || F.moderatorSet) && cmd === "!set" && parts[1]) {
        ...
        if (sec>0){ state.remainingSeconds = sec; render(); showEvent("Set timer", parts[1]); }
      }
      if ((F.broadcasterIncrease || F.moderatorIncrease) && cmd === "!increase") {
        const min = Math.max(0, Number(parts[1] || 1)||1);
        addSeconds(min*60, `+${min*60}s`, "manual");
      }
      if ((F.broadcasterDecrease || F.moderatorDecrease) && cmd === "!decrease") {
        const min = Math.max(0, Number(parts[1] || 1)||1);
        applyDelta(-(min*60), {kind:"manual", message:`-${min*60}s`});
        showEvent("Decrease", `-${min*60}s`);
      }
      if ((F.broadcasterPause || F.moderatorPause) && cmd === "!pause") { pauseTimer(); showEvent("Paused",""); }
      if ((F.broadcasterStart || F.moderatorStart) && cmd === "!start") {
        if (parts[1]) state.remainingSeconds = parseHMS(parts[1]) || state.remainingSeconds;
        startTimer(); showEvent("Started","");
      }
      if ((F.broadcasterStop || F.moderatorStop) && cmd === "!stop") { stopTimer(); showEvent("Stopped",""); }
      if ((F.broadcasterRevert || F.moderatorRevert) && cmd === "!revert") { revert(Number(parts[1]||1)); }
    }
  }
  ...
  if (listener === "subscriber-latest") {
    ...
    const rawTier = String(ev?.tier || ev?.plan || ev?.tier_plan || ev?.subscriptionPlan || "").toLowerCase();
    let tier = "Other";
    if (rawTier.includes("3000") || rawTier.includes("tier3")) tier = "T3";
    else if (rawTier.includes("2000") || rawTier.includes("tier2")) tier = "T2";
    else if (rawTier.includes("1000") || rawTier.includes("tier1") || rawTier.includes("prime")) tier = "T1";
    let add = 0;
